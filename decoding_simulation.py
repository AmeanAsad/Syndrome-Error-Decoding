...
def stochastic_simulation(word_limit, num_trials, n_start, n_ceiling):
    ...
    percentage_matrix = np.zeros((table_length-1,(n_ceiling - n_start)), dtype = float)
    
    while(text_length < word_limit):
        text_length = text_length + count_increase_factor 
        word_count.append(text_length)
        n_values[row_index,:], percentage_matrix[row_index,:] = single_decoding_simulation(text_length, num_trials, n_start,n_ceiling)
        row_index +=1
...
def visualization(word_limit, num_trials):
    ...
    result_matrix, word_count, percentage_matrix = stochastic_simulation(word_limit, num_trials, n_start, n_ceiling)
    ...
    for idx in range(n_start, n_ceiling):
        ...
        table_text[matrix_index,:] = [np.mean(percentage_matrix[:,matrix_index]),np.std(percentage_matrix[:,matrix_index])]
